...
Game::Game(int screenWidth, int screenHeight) :player(Player(Vector2{ 180,400 }, "assets\\sprites\\bluebird-downflap.png"))
{
	...
	fxFly = LoadSound("assets\\audio\\wing.wav");
	fxPass = LoadSound("assets\\audio\\swoosh.wav");
	fxDie = LoadSound("assets\\audio\\hit.wav");
	fxHit = LoadSound("assets\\audio\\die.wav");
}
...
void Game::Update()
{
	...
	if (isPLaying)
	{
		...
		else if ((player.pos.x + 50 >= ptrCurrentPipe->posTop.x - 100 && player.pos.x + 50 <= (ptrCurrentPipe->posTop.x) &&
